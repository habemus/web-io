...
module.exports = function (webIO) {
  ...
  function itemMetaData(dataItem, options) {
    ...
    try {
      var path = webIO.truncateFsRoot(
        replaceExt(dataItem._meta('path'), '.html')
      );
  ...
  function renderMarkdown(dataItem, options) {
    if (mime.lookup(dataItem._meta('path')) === MARKDOWN_MIME) {
      ...
      .catch(err => {
        throw new errors.RenderError(dataItem._meta('path'), err);
      });
    }
  }
