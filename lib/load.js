...
exports.loadTemplate = function (templateName, options) {
  
  return new Bluebird((resolve, reject) => {
    var absoluteTemplatePath = this.fsPrependPath(templateName);
  ...
  .then((res) => {
    ...
    if (aux.isMarkdown(templateName)) {
      parsePromise = this.parseMarkdown(res, options);
    } else if (aux.isHTML(templateName)) {
      parsePromise = this.parseHTML(res, options);
    } else {
      parsePromise = this.parseHTML(res, options);
    }
  ...
  .then((parsed) => {
    var path = replaceExt(templateName, '.html');
    var url  = this.websitePrependPath(path);
