...
exports.loadDataReferences = function (dataItem, options) {
  ...
  var references = Object.keys(dataItem).filter(prop => {
    
    var isReference = DATA_PROP_RE.test(prop);
    var hasBeenLoaded = dataItem._meta('reference_loaded_' + prop) || false;
    var force = options.force;
    
    if (isReference) {
      if (force || !hasBeenLoaded) {
        // load only if force is set or if 
  ...
  return Bluebird.all(references.map(referenceProperty => {
    ...
    var query = dataItem[referenceProperty];
    ...
    dataItem._meta('reference_loaded_' + referenceProperty, true);

    return this.execDataLoader(dataSource, query).then((result) => {
      // set target data property
      dataItem[dataProperty] = result;
    });
  }))
  .then(() => {
    return dataItem;
